...
test('Digital Library catalog loads NCERT Class 7 Chapter 2 book', () => {
  const book = getLibraryBook('ncert-7-ch2');
  assert.ok(book, 'NCERT book should be registered');
  ...
  assert.ok(books.length >= 1, 'Should return at least 1 book');
});
