...
// Since credentials are pinned per lesson (agora/credentials.ts), mintTokens
// takes the pair explicitly rather than reading config — so no env is needed.
const CREDS = {
...
test('RTC token embeds the exact channel it was minted for', () => {
  const rtc = decode(mintTokens(CHANNEL, UID, CREDS).rtcToken);
...
test('RTM token is a distinct token bound to the same uid', () => {
  const { rtcToken, rtmToken } = mintTokens(CHANNEL, UID, CREDS);
...
test('a numeric-string uid and its Number() form mint the same RTC uid field', () => {
  const a = decode(mintTokens(CHANNEL, '900123', CREDS).rtcToken);
  const b = decode(mintTokens(CHANNEL, String(Number('900123')), CREDS).rtcToken);
